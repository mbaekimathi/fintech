...
DARAJA_NAV = [
    {"name": "Daraja app", "url": "core:daraja", "icon": "key"},
...
DARAJA_URLS = {item["url"] for item in DARAJA_NAV} | {"core:settings"}
HR_NAV = [
...
def _section_items(user):
    ...
    if user.can_manage_ledger() or user.effective_role == User.Role.IT_SUPPORT:
...
def shell(request):
    ...
    if user and user.is_authenticated and not user.is_pending:
        ...
        if current in DARAJA_URLS and user.can_manage_daraja():
        ...
        can_review_money_requests = user.effective_role in REVIEW_ROLES or (
